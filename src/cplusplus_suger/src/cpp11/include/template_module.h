...
class TempPartial<T1, int> {
 public:
  void Print(const T1 v1, const int v2) {
    std::cout << "Partial specialization int class: " << v1 << ' ' << v2
              << std::endl;
  }
};
...
class TempPartial<int, float> {
 public:
  void Print(const int v1, const float v2) {
    std::cout << "Specialization int and float class: " << v1 << ' ' << v2
              << std::endl;
  }
};
...
template <>
inline void FuncTempPrint<10>() {
  std::cout << "Function specialization: " << 10 << std::endl;
}

template <>
inline void FuncTempPrint<100>() {
