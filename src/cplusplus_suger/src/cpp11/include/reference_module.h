...
class First {
 public:
  First() : num_(new int(0)) { std::cout << "First construct!" << std::endl; }
...
class Second {
 public:
  Second() : first_() { std::cout << "Second construct!" << std::endl; }
  Second(Second&& second) : first_(std::move(second.first_)) {
    std::cout << "Second move construct!" << std::endl;
  }
  void Print() { std::cout << "Second hello world!" << std::endl; }
...
class RefQualifierDemo {
 public:
  ...
  int GetNumLvalRef() & { return this->lnum_; }
  int GetNumRvalRef() && { return this->rnum_; }
...
class Demo {
 public:
  Demo() : num_(new int(0)) { std::cout << "Construct!" << std::endl; }
