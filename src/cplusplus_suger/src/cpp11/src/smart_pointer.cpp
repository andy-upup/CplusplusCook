...
static void DeleteInt(int *ptr) { delete[] ptr; }
...
void InitSharedPtr() {
  ...
  // error, not allow to initialize two shared pointers with the same ordinary
  // pointer std::shared_ptr<int> p8(ptr);
...
void UseSharedPtrReset() {
  struct Demo {
    Demo() { std::cout << "Construct demo!" << std::endl; }
    ~Demo() { std::cout << "Destruct demo! Now num is: " << num << std::endl; }
    int num;
  };
...
void InitUniquePtr() {
  ...
  // deferent from shared_ptr, unique_ptr can only relese resources through
  // function object
