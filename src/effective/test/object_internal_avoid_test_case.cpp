...
TEST(test_effective, object_internal_avoid_unit_test) {
  ...
  std::shared_ptr<effective::RectData> prect =
      std::make_shared<effective::RectData>(rect);
  const effective::Rectangle rectangle(prect);

  std::cout << "upper_left_point is: " << rectangle.GetUpperLeft().GetX() << " "
            << rectangle.GetUpperLeft().GetY() << std::endl;
